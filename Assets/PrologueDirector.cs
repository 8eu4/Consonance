...
public class PrologueDirector : MonoBehaviour
{
    ...
    IEnumerator PlaySequence()
    {
        ...
        // [MODIFIKASI] Matikan Rigidbody (Jadi Patung)
        if (playerRigid != null)
        {
            playerRigid.isKinematic = true; 
            // Note: linearVelocity hanya untuk Unity 6. Jika pakai versi lama ganti jadi .velocity
        ...
        // [MODIFIKASI] Nyalakan Rigidbody (Aktifkan Fisika)
        if (playerRigid != null) playerRigid.isKinematic = false;
        
        // [FIXED] Bagian ini yang tadi error sudah diperbaiki
        foreach (var script in scriptsToDisable) {
            if (script != null) {
                script.enabled = true; // Nyalakan script (Movement, CamRotation, dll)

                // Cek khusus untuk CamRotation, panggil UpdateOrientation saja
                if (script is CamRotation camScript) 
                {
                    camScript.UpdateOrientation(); 
                    // Baris SyncRotation() DIHAPUS karena script temanmu tidak punya fungsinya.
                }
            }
        }
